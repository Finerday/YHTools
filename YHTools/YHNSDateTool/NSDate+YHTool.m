...
+ (NSString *)yh_getCurrentDate:(NSDate *)date isAdvance:(BOOL )isAdvance{
    ...
    if (isAdvance == YES) {
        ...
        NSArray *a = [[array lastObject] componentsSeparatedByString:@":"];
        NSString *str = [a firstObject];
        if ([str intValue] == 23) {
            m = @"23:59";
        }else{
            NSString *dateStr1 = [formatter  stringFromDate: [NSDate dateWithTimeInterval:90*60 sinceDate:[NSDate date]]];
            NSArray *array1 = [dateStr1 componentsSeparatedByString:@"-"];
            m = [array1 lastObject];
        }
    }else{
        NSString *dateStr1 = [formatter  stringFromDate: [NSDate dateWithTimeInterval:60*60 sinceDate:[NSDate date]]];
        NSArray *array1 = [dateStr1 componentsSeparatedByString:@"-"];
        m = [array1 lastObject];
    }
   ...
   return [NSString stringWithFormat:@"%@ %@-%@",[array firstObject],weekStr,m];
}
